...
//spatially expensive with pointer implementation
...
void printPath(State *finalState) {
    ...
    State *currState = finalState;

	//we want to print the paths in order from initial state to final state
    stack<State*> statesInPath;

	//traverse back to initial state getting all parent states
    while (currState != nullptr) {
        statesInPath.push(currState);
        currState = currState->parent;
    }

	//print in order from stack
    while (!statesInPath.empty()) {
        statesInPath.top()->show();
        statesInPath.pop();
        cout<<endl;
    }
}
	...
	while (!states.empty()) {
        ...
        if (isSolved(currState)) {
            ...
            for (string move : possibleMoves) {
				...
				nextState->x = currState->x;
				nextState->y = currState->y;
				nextState->z = currState->z;
